<script lang="ts">
  ...
  function startDrag(e: PointerEvent) {
    if (currentState === "running" || currentState === "paused" || isStopping)
      return;
  ...
  function updateDragPosition(e: PointerEvent) {
    ...
    // Play tick sound based on drag distance to prevent boundary oscillation
    // 0.1 threshold means exactly 10 ticks across the full bar.
    if (Math.abs(percentage - lastTickTime) >= 0.05 && percentage > 0) {
      playTick();
      lastTickTime = percentage;
    }
  }
  ...
  function onDragEnd(e: PointerEvent) {
    ...
    if (totalTimeSet > 0) {
      ...
      if (roundedMins === 0)
        roundedMins = Math.max(1, Math.round(totalTimeSet / 60));
...
>
  ...
  <button
    ...
    onclick={currentState === "running" || currentState === "paused"
      ? togglePause
      : undefined}
...
<style lang="scss">
  ...
  .progress-gradient {
    ...
    background: linear-gradient(
      to right,
      color-mix(in srgb, $color-accent 0%, transparent) 0%,
      color-mix(in srgb, $color-accent 10%, transparent) 100%
    );
