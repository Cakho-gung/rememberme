<script lang="ts">
          ...
          items.push(
            ...Array.from(
              section.querySelectorAll<HTMLElement>(
                "button:not(.section-header), input",
              ),
  ...
  // Definition of every shortcut with its DEFAULT combo. The actual displayed
  // value is loaded from localStorage (so user changes persist across reopen).
  ...
  const shortcutDefs: ShortcutDef[] = [
    {
      ...
      group: "App",
      default: getSavedToggleShortcut(),
    },
    {
      id: "theme-toggle",
      label: "Toggle Theme",
  ...
  // ('globalToggleShortcut'), so use its default (which is the saved value) as-is.
  // Every other shortcut is loaded from localStorage via getLocalShortcut.
  let shortcuts = $state<Shortcut[]>(
    shortcutDefs.map((def) => {
      const stored =
        def.id === "global-toggle"
          ? def.default
          : getLocalShortcut(def.id, def.default);
      const parsed = parseShortcutStr(stored);
      return {
        id: def.id,
        label: def.label,
  ...
  // ── Focus Animation ──
  let focusAnimationEnabled = $state(
  ...
  function toggleFocusAnimation() {
    ...
    localStorage.setItem(
      ...
      focusAnimationEnabled.toString(),
    );
  }
  ...
  let currentToastDuration = $state(
    localStorage.getItem("toastDuration")
      ? parseInt(localStorage.getItem("toastDuration") as string)
      : 3000,
  );
...
>
  ...
  <div class="settings-header" onpointerdown={startDragging}>
    <button class="settings-back-btn" onclick={onClose} aria-label="Back">
      <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
        <path
          d="M9.96967 3.46967C10.2626 3.76256 10.2626 4.23744 9.96967 4.53033L6.49999 8L9.96967 11.4697C10.2626 11.7626 10.2626 12.2374 9.96967 12.5303C9.67678 12.8232 9.20191 12.8232 8.90901 12.5303L4.90901 8.53033C4.61612 8.23744 4.61612 7.76256 4.90901 7.46967L8.90901 3.46967C9.20191 3.17678 9.67678 3.17678 9.96967 3.46967Z"
        />
      </svg>
    </button>
  ...
  <div class="settings-body">
          ...
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Hover, collapse, and theme switch sounds...</span
              >
            </div>
            <!-- Toggle Switch -->
            <button
    ...
    <div class="settings-section">
      <button
        class="section-header"
        onclick={() => (uiScaleOpen = !uiScaleOpen)}
      ...
      >
        ...
        <svg
          class="chevron {uiScaleOpen ? 'open' : ''}"
          width="12"
          height="12"
          viewBox="0 0 16 16"
          fill="currentColor"
        ...
        <div class="section-body" transition:fly={{ y: -4, duration: 150 }}>
          ...
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Animate gradient border on app focus.</span
              >
            </div>
            <!-- Toggle Switch -->
            <button
          ...
          <div class="setting-row">
            ...
            >
              {#each [1000, 3000, 5000, 8000] as number[] as preset}
                <button
                  class="scale-btn {currentToastDuration === preset
                    ? 'active'
                    : ''}"
        ...
        <div class="section-body" transition:fly={{ y: -4, duration: 150 }}>
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Set the maximum duration for the focus timer.</span
              >
            </div>
            <!-- Segmented scale picker -->
            <div class="scale-selector" role="group" aria-label="Timer Limit">
              {#each ["20s", "60m", "2h", "4h", "8h"] as TimerPreset[] as preset}
                <button
                  class="scale-btn {currentTimerPreset === preset
                    ? 'active'
                    : ''}"
    ...
    <div class="settings-section">
      ...
      >
        ...
        <svg
          class="chevron {shortcutsOpen ? 'open' : ''}"
          width="12"
          height="12"
          viewBox="0 0 16 16"
          fill="currentColor"
      ...
      {#if shortcutsOpen}
        <div class="section-body" transition:fly={{ y: -4, duration: 150 }}>
          <p class="shortcut-hint">
            Click a shortcut to record a new key combo. Press <kbd>Esc</kbd> to cancel.
          </p>
          ...
          <div class="shortcut-group">
            ...
            {#each shortcuts.filter((s) => s.group === "App") as shortcut}
              <div class="shortcut-row">
                ...
                <button
                  class="shortcut-key-btn {recordingId === shortcut.id
                    ? 'recording'
                    : ''}"
                  onclick={() =>
        ...
        <div class="section-body" transition:fly={{ y: -4, duration: 150 }}>
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Copy all note as markdown to clipboard.</span
              >
            </div>
            <button
              class="export-btn"
          ...
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Save note as .md file into Documents/RememberMe/exports/.</span
              >
            </div>
            <button
              class="export-btn"
...
<style lang="scss">
  ...
  @keyframes pulse-border {
    0%,
    100% {
      box-shadow: 0 0 0 0 rgba(128, 128, 128, 0.3);
    }
    50% {
